    ...
    <PageBody><Stack gap="lg">
      ...
      {loading ? <Group justify="center" mih={200}><Loader size="sm" /></Group> : !error && <Table.ScrollContainer minWidth={galleryId ? 700 : 560}><Table verticalSpacing="md" highlightOnHover aria-label={galleryId ? "Gallery item жагсаалт" : "Gallery жагсаалт"}>
        ...
        <Table.Tbody>{visible.map((row) => <Table.Tr key={row.id}>
          {"imageId" in row && <Table.Td w={116}><Image src={fileUrl({ id: row.imageId, originalName: row.originalName }, 120)} w={96} h={72} fit="cover" alt={galleryPlainText(row.title) || row.originalName} /></Table.Td>}
          <Table.Td maw={280}>{"name" in row ? <Stack gap={2}><Text component={Link} to={`/galleries/${row.id}`} c="blue" fw={500} lineClamp={2}>{row.name}</Text><Text size="xs" c="dimmed" lineClamp={1}>{row.key}</Text></Stack> : <Stack gap={2}><Text size="sm" fw={500} lineClamp={2}>{galleryPlainText(row.title) || row.originalName}</Text><Text size="xs" c="dimmed" lineClamp={1}>{galleryPlainText(row.label)}</Text></Stack>}</Table.Td>
