...
export function VehicleEditPage() {
  ...
  return <Stack>{notice && <Alert color="teal" role="status" withCloseButton onClose={() => setNotice("")}>{notice}</Alert>}
    <VehicleForm key={`${id ?? "new"}:${row?.updatedAt ?? ""}`} row={row} lookups={lookups.data} back={back} onSaved={(result) => {
      setNotice("Амжилттай хадгаллаа."); setRow(result);
      if (!id) void navigate(`/vehicles/${result.id}/edit?${new URLSearchParams({ returnTo: back })}`, { replace: true });
    }} />
  </Stack>;
}
