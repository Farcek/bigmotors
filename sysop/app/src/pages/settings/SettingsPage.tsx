...
import { ADMIN_SETTING_KEYS, loadPublishedPages, settingsError } from "./model";

export function SettingsPage() {
  const [reload, setReload] = useState(0); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [data, setData] = useState<{ entries: Settings.Entity[]; pages: Pages.ListEntity[] }>();
  useEffect(() => {
    ...
    void (async () => {
      ...
      const pages = await loadPublishedPages(offset => apiClient.call(Pages.list, { query: { status: "published", limit: 100, offset } }, { signal: controller.signal }), controller.signal);
      if (!controller.signal.aborted) setData({ entries, pages });
  ...
  return <Box maw={850}><Stack>
    ...
    <PageBody>{loading ? <Group justify="center" mih={180}><Loader size="sm" /></Group> : error ? <Alert color="red" role="alert">{error}</Alert> : data &&
      <SettingsForm key={reload} entries={data.entries} pages={data.pages} onBusy={setBusy} onSave={entries => apiClient.call(Settings.save, { body: { entries } })} />}
    </PageBody>
  </Stack></Box>;
}
