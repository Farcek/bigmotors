...
async function main() {
  if (process.env.NODE_ENV === "production") throw new Error("Demo vehicles are development-only.");
  const origin = `http://127.0.0.1:${new ConfigSysop(process.env).PORT}`;
  const maxBytes = new ConfigFiles(process.env).FILE_UPLOAD_MAX_BYTES;
  ...
  console.log("20 demo vehicles ready. No records were published; existing vehicles were not overwritten.");
}
