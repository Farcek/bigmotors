...
import { vehicleFilters } from "@bigmotors/core";
import { z } from "zod";
...
export namespace HomeProductGroups {
  ...
  export const entity = z.object({ ...entityFields, title: z.string().min(1).max(255), imageId: z.string().uuid().nullable(), filters: vehicleFilters }).strict();
  export const createBody = z.object({ ...createFields, title: z.string().trim().min(1).max(255), imageId: z.string().uuid().nullable().optional(), filters: vehicleFilters }).strict();
