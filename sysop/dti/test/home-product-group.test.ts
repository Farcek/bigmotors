...
test("home group contracts restrict filters to the public vehicle search vocabulary", () => {
  ...
  for (const filters of [{ page: "2" }, { publicationStatus: "draft" }, { sql: "anything" }, { engine_min: "3000", engine_max: "2000" }, { year_max: String(new Date().getUTCFullYear() + 1) }, { fuel: "invalid" }, { mileage_min: 0 }, []]) assert.equal(HomeProductGroups.createBody.safeParse({ title: "test", filters }).success, false);
