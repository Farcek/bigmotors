...
import { bigint, boolean, check, foreignKey, index, integer, pgTable, text, timestamp, unique, uuid, varchar, type AnyPgColumn, type PgTableExtraConfigValue } from "drizzle-orm/pg-core";
...
export const products = pgTable("products", {
  ...
  content: text("content"), mainImageId: uuid("main_image_id"),
  itemTitle: varchar("item_title", { length: 255 }), itemDesc: varchar("item_desc", { length: 512 }), itemImageId: uuid("item_image_id"),
  ...
  foreignKey({ name: "products_main_image_owner_fk", columns: [t.id, t.mainImageId], foreignColumns: [productImages.productId, productImages.id] }).onDelete("restrict"),
  foreignKey({ name: "products_item_image_owner_fk", columns: [t.id, t.itemImageId], foreignColumns: [productImages.productId, productImages.id] }).onDelete("restrict"),
...
export const productImages = pgTable("product_images", {
  ...
  filePath: text("file_path").notNull().unique(), originalName: text("original_name").notNull(),
  title: varchar("title", { length: 255 }), description: varchar("description", { length: 512 }),
  sortOrder: integer("sort_order").notNull().default(0), ...timestamps(),
}, (t) => [
  unique("product_images_product_id_id_unique").on(t.productId, t.id),
  index("product_images_order_idx").on(t.productId, t.sortOrder, t.id),
  nonNegative("product_images_sort_order_check", t.sortOrder), nonBlank("product_images_path_nonblank", t.filePath),
]);
