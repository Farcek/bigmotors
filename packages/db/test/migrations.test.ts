...
test("gallery key migration backfills existing rows without losing items", async () => {
  const folder = await mkdtemp(join(tmpdir(), "bigmotors-gallery-upgrade-"));
  const db = new PGlite();
  try {
    await mkdir(join(folder, "meta"));
    const journal = JSON.parse(await readFile(new URL("../migrations/meta/_journal.json", import.meta.url), "utf8"));
    ...
    for (const entry of entries) {
      await writeFile(join(folder, `${entry.tag}.sql`), await readFile(new URL(`../migrations/${entry.tag}.sql`, import.meta.url)));
    }
    const orm = drizzle(db);
    await migrate(orm, { ...migrationConfig, migrationsFolder: folder });
